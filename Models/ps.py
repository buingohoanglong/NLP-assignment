...
def lf2ps(logical_form):
    ...
    for component in lf_components:
        ...
        fields[temp[0] if temp[0] != 'AGENT' else 'BUS-NAME'] = temp[3]

    # build procedural semantic
    # tables format in data base
    bus = {'NAME': 'BUS', 'DATA': {'BUS-NAME': ''}}
    atime = {'NAME': 'ATIME', 'DATA': {'BUS-NAME': '', 'TO-LOC': '', 'TO-TIME': ''}}
    dtime = {'NAME': 'DTIME', 'DATA': {'BUS-NAME': '', 'FROM-LOC': '', 'FROM-TIME': ''}}
    runtime = {'NAME': 'RUN-TIME', 'DATA': {'BUS-NAME': '','FROM-LOC': '','TO-LOC': '', 'RUN-TIME': ''}}
    for f in fields:
        for table in [bus, atime, dtime, runtime]:
            if f in table['DATA']:
